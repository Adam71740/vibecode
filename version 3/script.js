...
let index = 0;
function type(){ if(index<text.length){ typedEl.textContent += text.charAt(index); index++; setTimeout(type,50); } }
type();

// ==========================
// STARFIELD / PARTICLES / PLANETS / SCROLL REACTIVE
// ==========================
const canvas=document.getElementById("starfield"), ctx=canvas.getContext("2d");
let stars=[], planets=[], shootingStars=[], mouseParticles=[], scrollSpeed=0;
const STAR_COUNT=3000, PLANET_COUNT=10;
...
function initPlanets(){ planets=[]; for(let i=0;i<PLANET_COUNT;i++){ planets.push({ x:Math.random()*canvas.width, y:Math.random()*canvas.height, r:Math.random()*50+20, angle:Math.random()*Math.PI*2, speed:(Math.random()*0.02)+0.002 }); } }
...
window.addEventListener("scroll",()=>{ scrollSpeed=window.scrollY/500; });
...
setInterval(newShootingStar,1200);

document.querySelectorAll("button").forEach(btn=>{
  btn.addEventListener("click",e=>{ for(let i=0;i<100;i++){ mouseParticles.push({x:e.clientX, y:e.clientY, r:Math.random()*5+1, alpha:1}); } });
});
...
function animate(){
  ...
  // Stars
  stars.forEach(s=>{
    s.z-=2+scrollSpeed*5; if(s.z<=0){ s.x=Math.random()*canvas.width; s.y=Math.random()*canvas.height; s.z=canvas.width; s.o=Math.random(); s.size=Math.random()*2; }
  ...
  // Planets orbit
  planets.forEach(p=>{
    p.angle+=p.speed+scrollSpeed*0.01; const px=p.x+Math.cos(p.angle)*50; const py=p.y+Math.sin(p.angle)*50;
